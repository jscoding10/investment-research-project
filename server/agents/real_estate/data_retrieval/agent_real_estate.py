...
# def data_retrieval_agent(state: RealEstateResearchState) -> dict:
#     """
#     Dedicated agent that fetches and validates data from BOTH Zillow and RentCast APIs.
#     - Fails with clear HTTP 400 if either API does not return valid property data.
#     - Stores raw responses in state.zillow_raw and state.rentcast_raw for reuse.
#     - Runs after basic address sanitization.
#     """
#     address = state.address.strip()
#     if not address:
#         raise HTTPException(400, detail="Address is required")

#     logger.info(f"Data Retrieval Agent: fetching Zillow + RentCast for {address}")
...
#         if not response.ok:
#             errors.append(f"Zillow API failed: {response.status_code} - {response.text[:100]}")
#         else:
#             data = response.json()
#             if "propertyDetails" not in data or not data["propertyDetails"].get("zpid"):
#                 errors.append("Zillow returned no valid property details (missing zpid)")
#             else:
#                 state.zillow_raw = data
#                 state.zpid = data["propertyDetails"]["zpid"]
#                 logger.info(f"Zillow success: ZPID {state.zpid}")

#     except Exception as e:
#         errors.append(f"Zillow fetch exception: {str(e)}")
...
#         )
#         rc_response = requests.get(rc_url, timeout=10)

#         if not rc_response.ok:
#             errors.append(f"RentCast API failed: {rc_response.status_code} - {rc_response.text[:100]}")
#         else:
#             rc_data = rc_response.json()
#             if "data" not in rc_data or "property" not in rc_data["data"]:
#                 errors.append("RentCast returned no valid property data")
#             else:
#                 state.rentcast_raw = rc_data
#                 logger.info("RentCast success")

#     except Exception as e:
#         errors.append(f"RentCast fetch exception: {str(e)}")

#     # Final check
#     if not state.zillow_raw or not state.rentcast_raw:
#         error_msg = "Failed to retrieve complete property data.\n" + "\n".join(errors)
#         logger.error(error_msg)
#         raise HTTPException(status_code=400, detail=error_msg + "\nPlease verify the address and try again.")

#     state.data_retrieved = True
#     return {"data_retrieved": True}
...
#     """
#     Dedicated agent that fetches and validates data from BOTH Zillow and RentCast APIs.
#     - Logs full raw responses at INFO level when successful
#     - Fails with clear HTTP 400 if either API does not return valid property data
#     - Stores raw responses in state.zillow_raw and state.rentcast_raw for reuse
#     """
#     address = state.address.strip()
...
#         if not response.ok:
#             error_text = f"Zillow API failed: {response.status_code} - {response.text[:300]}"
#             errors.append(error_text)
#             logger.info(error_text)  
#         else:
#             try:
#                 data = response.json()
#                 logger.info("Zillow API call succeeded")
#                 # logger.info(f"Zillow raw response: {data}")  # ← full JSON at INFO level
...
#                 errors.append(f"Zillow response is not valid JSON: {str(json_err)}")
#                 logger.info(f"Zillow JSON decode error: {json_err}")

#     except Exception as e:
#         errors.append(f"Zillow fetch exception: {str(e)}")
#         logger.info(f"Zillow request exception occurred: {str(e)}")
...
#         if not rc_response.ok:
#             error_text = f"RentCast API failed: {rc_response.status_code} - {rc_response.text[:300]}"
#             errors.append(error_text)
#             logger.info(error_text)
#         else:
#             try:
#                 rc_data = rc_response.json()
#                 logger.info("RentCast API call succeeded")
...
#             except ValueError as json_err:
#                 errors.append(f"RentCast response is not valid JSON: {str(json_err)}")
#                 logger.info(f"RentCast JSON decode error: {json_err}")

#     except Exception as e:
#         errors.append(f"RentCast fetch exception: {str(e)}")
#         logger.info(f"RentCast request exception occurred: {str(e)}")

#     # Final check
#     if not state.zillow_raw or not state.rentcast_raw:
#         error_msg = "Failed to retrieve complete property data.\n" + "\n".join(errors)
#         logger.info(f"Data retrieval failed: {error_msg}")
#         raise HTTPException(
#             status_code=400,
#             detail=error_msg + "\nPlease verify the address and try again."
#         )
...
#     z_prop = state.zillow_raw.get("propertyDetails", {}) if state.zillow_raw else {}
#     rc_prop = state.rentcast_raw.get("data", {}).get("property", {}) if state.rentcast_raw else {}

#     reso_facts = z_prop.get("resoFacts", {})

#     # If there is no user input than override the purchase price and if not estimated rent than override with return result

#     # Add Homeowners monthly and Tax Annual if exist

#     state.tax_annual = float(reso_facts.get("taxAnnualAmount", 0))
#     state.hoa_monthly = float(reso_facts.get("monthlyHoaFee", 0))
#     state.zestimate = float(z_prop.get("zestimate", 0))
#     state.rent_estimate = float(rc_prop.get("rentEstimate", {}).get("value", 0))

#     logger.info(f"Extracted from Zillow: tax_annual={state.tax_annual}, hoa_monthly={state.hoa_monthly}, zestimate={state.zestimate}")
#     logger.info(f"Extracted from RentCast: rent_estimate={state.rent_estimate}")
    
#     logger.info(f"Data retrieval successful - ZPID: {state.zpid}")
#     return {
#     "data_retrieved": True,
#     "zpid": state.zpid,
...
#     """
#     Fetch + validate + extract property data from Zillow (RapidAPI) and RentCast.
...
#     # logger.info(f"RAW DATA: ${zillow_raw}")
